...
-type option()     :: {'timeout', timeout()}
		    | {'debug', [debug_flag()]}
		    | {'spawn_opt', [proc_lib_mod:spawn_option()]}.
-type options()    :: [option()].
...
do_spawn(GenMod, link, Mod, Args, Options) ->
    ...
    proc_lib_mod:start_link(?MODULE, init_it,
			[GenMod, self(), self(), Mod, Args, Options], 
			Time,
			spawn_opts(Options));
do_spawn(GenMod, _, Mod, Args, Options) ->
    Time = timeout(Options),
    proc_lib_mod:start(?MODULE, init_it,
		   [GenMod, self(), self, Mod, Args, Options], 
		   Time,
		   spawn_opts(Options)).
...
init_it(GenMod, Starter, Parent, Name, Mod, Args, Options) ->
	...
	{false, Pid} ->
	    proc_lib_mod:init_ack(Starter, {error, {already_started, Pid}})
    end.
...
stop(Process, Reason, Timeout)
  when Timeout =:= infinity; is_integer(Timeout), Timeout >= 0 ->
    Fun = fun(Pid) -> proc_lib_mod:stop(Pid, Reason, Timeout) end,
    do_for_proc(Process, Fun).
